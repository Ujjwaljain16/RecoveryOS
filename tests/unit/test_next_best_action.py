...
def test_best_evi_action_wins():
    candidates = _all_six(RETRY_NOW=500, RETRY_LATER=200, ALT_ROUTE=100, REMINDER=50, ESCALATE=10, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=8000)
...
def test_below_floor_actions_rejected_in_favor_of_a_clearing_one():
    candidates = _all_six(RETRY_NOW=5, RETRY_LATER=400, ALT_ROUTE=-10, REMINDER=-5, ESCALATE=-1, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=100, propensity_probability_bps=8000)
...
def test_do_nothing_selected_when_no_action_clears_floor():
    ...
    candidates = _all_six(RETRY_NOW=-500, RETRY_LATER=-300, ALT_ROUTE=-200, REMINDER=-150, ESCALATE=-9000, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=1000)
...
def test_do_nothing_wins_even_when_its_own_evi_is_exactly_the_floor():
    ...
    candidates = _all_six(RETRY_NOW=0, RETRY_LATER=0, ALT_ROUTE=0, REMINDER=0, ESCALATE=0, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=1000)
    assert result.chosen_action == "DO_NOTHING"
    assert result.cleared_floor is False
...
def test_retry_later_can_beat_retry_now():
    candidates = _all_six(RETRY_NOW=100, RETRY_LATER=900, ALT_ROUTE=50, REMINDER=10, ESCALATE=-100, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=8000)
    assert result.chosen_action == "RETRY_LATER"


def test_alt_route_can_beat_retry():
    candidates = _all_six(RETRY_NOW=100, RETRY_LATER=50, ALT_ROUTE=900, REMINDER=10, ESCALATE=-100, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=8000)
    assert result.chosen_action == "ALT_ROUTE"


def test_reminder_can_beat_retry():
    candidates = _all_six(RETRY_NOW=10, RETRY_LATER=5, ALT_ROUTE=1, REMINDER=900, ESCALATE=-100, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=8000)
    assert result.chosen_action == "REMINDER"


def test_escalate_can_beat_retry():
    candidates = _all_six(RETRY_NOW=10, RETRY_LATER=5, ALT_ROUTE=1, REMINDER=2, ESCALATE=900, DO_NOTHING=0)
    result = select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=8000)
...
def test_missing_do_nothing_candidate_raises():
    ...
    try:
        select_next_best_action(candidates, min_expected_value_paise=0, propensity_probability_bps=8000)
