...
**Separation of cognition and control.** Everything that touches money is deterministic, testable, and replayable without an LLM in the loop. The LLM is a *reasoning and explanation layer* bolted onto a *rules engine* — never the actuator. This is the single architectural decision that makes the whole system defensible under adversarial questioning ("what if the LLM hallucinates a retry on a ₹5L payment?" → it can't, because the LLM never calls the executor, and it doesn't even write a recommendation the policy engine consults — see below).

CORRECTED (Domain Audit finding F4/A2, pre-existing doc-code mismatch): this used to say the LLM "writes a recommendation record that the policy engine independently evaluates against hard-coded limits" — that is not what the code does. `services/recovery_engine/orchestrator.py:build_decision()` computes `chosen_action` from the certified propensity model + EVI + anomaly context alone; `services/policy_engine/rules.py`'s 10 rules take only `PaymentContext`/`CandidateContext`/`PolicyConfigContext` and never reference diagnosis, confidence, or root_cause anywhere (confirmed by grep — zero hits). `diagnosis_id` is threaded through only as an FK for audit/scheduling bookkeeping, never read back to influence a decision. This is a *stronger* guarantee than "propose then check," not a weaker one — it's independent parallel computation with zero data flow from diagnosis to policy, not a checked recommendation — but the TRD's own prose was overstating the LLM's role relative to what the code actually does, which is corrected here rather than left standing.
