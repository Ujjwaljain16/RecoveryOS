...
**Separation of cognition and control.** Everything that touches money is deterministic, testable, and replayable without an LLM in the loop. The LLM is a *reasoning and explanation layer* bolted onto a *rules engine*, with one narrow, bounded exception (AI recommendation fusion, §3.5) — the LLM is never the actuator, never chooses an amount/ID/provider parameter, and never authorizes an out-of-policy action, but it CAN change which already-cleared candidate wins, within tight, disclosed bounds. See §3.5, not this section, for the precise current mechanism.

CORRECTED (Domain Audit finding F4/A2, pre-existing doc-code mismatch): this used to say the LLM "writes a recommendation record that the policy engine independently evaluates against hard-coded limits" — that was not what the code did AT THE TIME. `services/recovery_engine/orchestrator.py:build_decision()` computed `chosen_action` from the certified propensity model + EVI + anomaly context alone; `services/policy_engine/rules.py`'s 10 rules take only `PaymentContext`/`CandidateContext`/`PolicyConfigContext` and never reference diagnosis, confidence, or root_cause anywhere (still confirmed by grep — zero hits, this part remains true). `diagnosis_id` is threaded through only as an FK for audit/scheduling bookkeeping, never read back to influence a decision by the rules themselves.

RE-CORRECTED (Re-Audit, commit `3bf04ea`, already merged before that audit ran — the previous audit was describing stale code): the paragraph above is no longer the whole picture. `services/recovery_engine/ai_fusion.py` + `orchestrator.py`'s `_apply_ai_fusion()` add a real, bounded mechanism, off by default (`Settings.ai_recommendation_fusion_enabled=false`), through exactly two paths — see §3.5 for the full description, not duplicated here to avoid this exact class of drift recurring:
...
RULES: tuple[PolicyRule, ...] = (
    EligibilityRule(),               # payment.status == 'failed' and not expired/already-recovered
    OptOutRule(),                     # customer.opted_out_at is None
    AIRiskSignalEscalationRule(),     # closed-set AI risk_flags -> ESCALATE (safety override, not a vote)
    ...
    MoneyExposureLimitRule(),          # mission's total outstanding exposure stays within max_money_exposure_paise
    EMandateRetryComplianceRule(),     # RBI/NPCI e-mandate retry-count ceiling (UPI Autopay/NACH only)
    AutopayExecutionWindowRule(),      # NPCI UPI Autopay non-peak execution window (UPI only)
    QuietHoursComplianceRule(),        # TRAI TCCCPR quiet hours, 21:00-09:00 IST (REMINDER only)
...
6. AI evidence / diagnosis / rationale           (informational only, unchanged since the diagnosis engine was introduced)
```

**The governing invariant: AI may resolve ambiguity, but it may never create permission.** Concretely, the investigator (a multi-round diagnosis loop, `services/diagnosis_engine/investigator.py`) now also emits a `RecoveryRecommendation` (`recommended_action` from the same closed 6-value set §3.1's candidates already use, `confidence`, a closed-set `risk_flags` enum, `recovery_rationale`) alongside its diagnosis. That recommendation can change `chosen_action` in exactly two bounded ways, both gated behind `Settings.ai_recommendation_fusion_enabled` (off by default):

1. **Tie-break.** Among candidates that already cleared the EVI floor *and* are individually policy-`ALLOW`ed on their own re-evaluation *and* sit within a small, fixed, disclosed relative tolerance (`Settings.ai_tie_break_tolerance_bps`, default 100 bps = 1%) of the deterministic winner's EVI, the AI's recommended action wins the tie — but only if `RecoveryRecommendation.confidence` also clears `Settings.ai_tie_break_min_confidence` (AI Architecture Gap Audit finding, closed: default 0.5, a pre-committed engineering safety/quality floor chosen *before* any measurement, never tuned post-hoc against a benchmark result — same discipline as the tolerance constant). A decisive winner (outside tolerance) is never overridden. A near-tied candidate the policy engine has independently rejected is never selected, even if the AI recommends it. A low-confidence recommendation is never selected either, even if it is near-tied and individually policy-`ALLOW`ed.
2. **Risk-flag escalation.** `AIRiskSignalEscalationRule` (an ordinary, pure `PolicyRule`, positioned right after `OptOutRule`) forces `ESCALATE` whenever `risk_flags` is non-empty — a safety intervention, so unlike every economic action it does not need to clear the EVI floor. The AI never decides `ESCALATE` itself; the rule does, from a bounded signal.

   Deliberately confidence-blind and economics-blind: the rule doesn't weigh `RecoveryRecommendation.confidence` or the candidate's EVI before escalating, on purpose — a safety intervention shouldn't need positive expected value or a high-confidence signal to be the right call, and making it confidence-weighted would open a real attack surface (an adversarial input crafted to keep the AI's stated confidence just under a threshold to suppress escalation). Re-Audit finding (MEDIUM), disclosed rather than silently left implicit: real-model evidence for this rule's actual escalation *rate* is thin and, as tracked so far, not even cleanly separable from `apps/api/routers/simulate.py`'s `safety_escalation` demo scenario, which deliberately seeds a forced `HIGH_FRAUD_RISK` recommendation rather than waiting for the live model to flag one organically — a real `recovery_recommendations` row, but a scripted one, not evidence of how often Gemini flags risk on its own. `tests/unit/test_ai_recommendation_adversarial.py`'s coverage is synthetic (constructed `risk_flags`, not live-model output), which proves the rule's *logic* is sound but says nothing about the LLM's real-world flagging rate at any real scale. Not a reason to weaken the rule — a reason a production deployment should monitor `ai_risk_escalations_total` against genuinely organic (non-demo) traffic before trusting any escalation-rate number, including any figure a prior audit pass of this same repo may have quoted, which itself goes stale the moment more demo/test traffic runs.
