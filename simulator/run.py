...
# gaps.md sec:C.2/C.3 -- val_random and test_scenario used to be generated by
# re-running build_simulator() with the SAME seed as train/test_random.
# DeterministicIdGenerator/SimRng have no cursor state, so two calls with an
# identical seed are byte-identical from index 0 -- val_random was a literal
# duplicate of train's leading episodes, and test_scenario was near-fully
# correlated with test_random (same underlying draws, only the scenario
# weights differed). These offsets decorrelate those two splits from the
# splits they were duplicating while staying fully deterministic from the
# top-level --seed/--test-seed CLI args (not a fresh random draw) -- so a run
# with the same --seed is still byte-reproducible, just no longer leaking.
VAL_RANDOM_SEED_OFFSET = 1_000_003
TEST_SCENARIO_SEED_OFFSET = 1_000_003
...
def build_simulator(
    ...
    scenarios = [
        NormalFailureScenario(
            baseline_failure_rate=sc_conf.get(
                "normal_rate", load_calibration().baseline_failure_rate
            )
        ),
        BankDegradationScenario(
...
def run_episode_mode(
    ...
    val_seed = seed + VAL_RANDOM_SEED_OFFSET
    test_scenario_seed = test_seed + TEST_SCENARIO_SEED_OFFSET

    train_eps = _gen_episodes(seed, n_train, "train")
    val_eps = _gen_episodes(val_seed, n_val, "val_random")
    ...
    manifest = builder.build(
        ...
        test_seed=test_seed,
        val_seed=val_seed,
        test_scenario_seed=test_scenario_seed,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "ISO-8601 UTC timestamp for the simulated clock's start (payments "
            "mode only), e.g. 2026-09-02T00:00:00Z. A canonical/evaluation run "
            "that feeds the LIVE pipeline (--output=db) MUST pass a recent "
            "value here -- every synthetic payment's failed_at is derived from "
            "this by ticking the clock forward, and services/recovery_engine/"
            "orchestrator.py's EligibilityRule blocks anything more than 7 "
            "real days old. Omit to keep the fixed 2026-08-20T09:00:00Z "
            "default (SimClock's own default, unchanged) -- safe for "
            "episodes-mode training data, which never reaches the live "
            "pipeline and so was never affected by this."
        ),
    )
    ...
    if args.start_time:
        try:
            sim_start_time = datetime.fromisoformat(args.start_time.replace("Z", "+00:00"))
    ...
    effective_start = sim_start_time.isoformat() if sim_start_time else "2026-08-20T09:00:00+00:00 (default)"
    print(
        f"[*] Starting RecoveryOS Simulation (n={args.n}, seed={args.seed}, "
        f"output={args.output}, sim_start_time={effective_start})..."
    )
