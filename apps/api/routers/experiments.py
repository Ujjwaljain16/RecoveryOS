...
from services.pipeline.baseline import PIPELINE_BASELINE_EXPERIMENT_ID

router = APIRouter()

MULTI_SEED_ARTIFACT_PATH = (
    Path(__file__).resolve().parents[3] / "tests" / "evaluation" / "artifacts" / "multi_seed_results.json"
)
...
def _phase8_baseline_experiment() -> dict:
    ...
    return {
        ...
        "recoveryos": {
            ...
            "recovery_rate_bps": round(statistics.mean([s["recovery_rate"] for s in seeds]) * 10_000),
